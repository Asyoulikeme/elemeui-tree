  ...
  <el-collapse accordion>
    <el-collapse-item v-for="(HttpActionReport,HttpActionReportIndex) in getHttpActionReportInterfaceConfig" :key="HttpActionReportIndex">
      <div slot="title" class="title-desc">
        <i class="header-icon el-icon-info"> HttpActionReport -- {{HttpActionReport.actionName}}</i>
      </div>
      <el-form :model="HttpActionReport">
        <el-form-item label="AppId" :label-width="formLabelWidth">
          <el-input v-model="HttpActionReport.appId" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="接口名称" :label-width="formLabelWidth">
          <el-input v-model="HttpActionReport.actionName" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="接口ID" :label-width="formLabelWidth">
          <el-input v-model="HttpActionReport.actionId" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="sqlSelect" :label-width="formLabelWidth">
          <el-input v-model="HttpActionReport.sqlSelect" autocomplete="off"></el-input>
        </el-form-item>
        ...
        <fieldset>
          <i class="el-icon-circle-plus" @click.stop="addConditions(HttpActionReportIndex)"> 添加</i>
          <legend>conditions[ ]</legend>
          <el-collapse accordion>
            <el-collapse-item v-for="(conditions,indexof) in HttpActionReport.conditions" :key="indexof">
              <div slot="title" class="title-desc">
                conditions -- {{indexof}}
              </div>
              <el-form-item label="appId" :label-width="formLabelWidth">
        ...
        <fieldset>
          <i class="el-icon-circle-plus" @click.stop="addSorts(HttpActionReportIndex)"> 添加</i>
          <legend>sorts[ ]</legend>
          <el-collapse accordion>
            <el-collapse-item v-for="(sorts,indexof) in HttpActionReport.sorts" :key="indexof">
              <div slot="title" class="title-desc">
                sorts -- {{indexof}}
              </div>
  ...
  export default {
    ...
    methods:{
      ...
      addConditions(indexOf){
        ...
        this.moduleConfig[this.index].interfaceConfig.HttpActionReport[indexOf].conditions.push(item)
      },
      addSorts(indexOf){
        let item = {
          sortId:'',
        ...
        this.moduleConfig[this.index].interfaceConfig.HttpActionReport[indexOf].sorts.push(item)
      }
    }
  };
</script>
